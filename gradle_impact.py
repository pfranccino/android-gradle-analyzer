...
class ImpactAnalyzer:
    ...
    def scan_and_build_graph(self):
        ...
        print(f"✓ {len(self.all_modules)} módulos encontrados\n")
        print("🔍 Construyendo grafo invertido de dependencias...")
    ...
    def generate_report(self):
        ...
        for lvl in sorted(by_level.keys()):
            ...
            mods  = sorted(by_level[lvl])
            lines.append(f"  Nivel {lvl} — {label} ({len(mods)}):")
            for m in mods:
                lines.append(f"    • {m}")
            lines.append("")
...
def main():
    ...
    parser.add_argument("project_root",   help="Ruta raíz del proyecto Android")
    parser.add_argument("target_module",  help="Módulo a analizar (ej: payments:common)")
    parser.add_argument(
        "--format",
